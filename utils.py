...
def make_dataset(root):
  ...
  for img in s3.list_objects(Bucket="mbenxsalha", Prefix="diffusion/pizza-not-pizza/pizza")["Contents"]:
    key = img["Key"]
    if is_image_file(key):
        data.append((key, 1))

  for img in s3.list_objects(Bucket="mbenxsalha", Prefix="diffusion/pizza-not-pizza/not_pizza")["Contents"]:
    key = img["Key"]
    if is_image_file(key):
        data.append((key, 0))
